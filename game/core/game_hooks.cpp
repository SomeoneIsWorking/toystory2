// game_hooks.cpp — the Toy Story 2 GameHooks vtable: the behaviour the PSX-generic framework calls
...
static bool ts2_hasNativeHandlerForEntry(Core*, uint32_t) { return false; }  // truthfully: none
static int  ts2_devAreaCount(Core*)            { return 0; }   // truthfully: no area index RE'd
static const char* ts2_devAreaName(Core*, int) { return ""; }  // "" == no sourced name
...
static void ts2_frameUpdate(Core*)         { unstood_up("frameUpdate (native frame loop)"); }
static void ts2_drawOTag(Core*, uint32_t)  { unstood_up("drawOTag (native frame loop)"); }
static int  ts2_schedStageBody(Core*, int, void*) { unstood_up("schedStageBody (PcScheduler)"); return 0; }
static bool ts2_schedFreshEntry(Core*, int, uint32_t, uint32_t) { unstood_up("schedFreshEntry (PcScheduler)"); return false; }
static void ts2_devWarpAreaLoad(Core*)     { unstood_up("devWarpAreaLoad (dev warp)"); }
