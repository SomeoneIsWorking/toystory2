...
Nobody is decompiling the PSX executable.
...
- **`mateusfavarin/tsr` is MIT and could legally be vendored — deliberately NOT.** A submodule sitting
  ...
  it growing.
- **`temisu/ancient` (BSD-2-Clause) is the one thing genuinely worth vendoring, and it is deferred until
  RE-08 needs it.** 468 lines of well-specified codec plus committed test vectors; vendoring buys
  correctness and vectors rather than much labour. It belongs in `toystory2/external/`, NOT in psxport —
  RNC ProPack is a Traveller's Tales concern and psxport is the generic framework.
