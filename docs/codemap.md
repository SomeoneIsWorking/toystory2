...
| Static recompilation | `tools/recomp_substrate.py`, `game/recomp_seeds.json` → `generated/` | 🟡 | RE-02/C014/I010. Input-hash-checked shipping emission produces 365 resident roots → 889 functions plus 22 modules / 308 functions / 75 TUs; a separate digest refuses drift. FMV entry `0x800D6628` is a binary- and live-proven module seed. Live execution now reaches shared BIOS `A0:0x25` from inside emitted FMV. |
...
| Framework seam — field clock | `game/sync/` (`field_clock.*`), `game/core/game_hooks.cpp` | ✅ | RE-10/C013/I013. After exact guest graphics init, arms host turns at the GPU field rate; each turn samples pad, invokes registered guest VBlank `0x80039D60`, advances SPU and presents. Live A/B changes a no-present watchdog into visible retail boot cards. This lifecycle is a derived GameRuntime responsibility, not another GameConfig field. |
...
| Process entry | `game/core/main.cpp` | 🟡 | Executed. Standard framework bring-up reaches verified crt0, generated gameMain, IRQ registration, disc DMA, visible boot cards and emitted FMV. It now fails fast at shared BIOS `A0:0x25`, with no recompilation miss at the old boundaries. |
