...
eleven ReadN phases, while current pin `54af32cb` retains the path under the full compile/static gate.
RE-10 then
...
| Registries | `tools/re_frontier.py` (shim), `tools/info.py`, `tools/catalog.py` | ✅ | The RE-frontier tracker is a SHIM onto the shared engine at `external/psxport/tools/port/re_frontier.py` — this repo grows no fork of it. `check` parses 18 entries with 0 hacks. C022/I018/resolved issue #19 close RE-06's discarded-host-input cause; investigating issue #20 records RE-17's observed record/replay divergence. `info.py` and `catalog.py` are COPIES (no hoisted engine exists for them yet) — see the known costs below. |
...
| Build | `CMakeLists.txt`, `cmake/toystory2_port.cmake`, framework `external/psxport/tools/check_cpp_style.py` | 🟡 | CMake accepts its supported host toolchain; maintainer verification explicitly selects Clang. It builds `toystory2_port`, one shared generated object set, and the independent-oracle boundary runner. CTest owns launcher-contract, RE gates including frame-fence and model-reset controls, boundary compare, format, structure and tidy. Generated sources stay ignored and use their required wrap/aliasing/tail-call flags. A clean Clang build against pin `54af32cb` builds both shipping targets and passes 13/13 CTest. |
