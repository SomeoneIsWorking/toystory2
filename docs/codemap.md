...
| Resident scene history | Observe the exact visible-list owner and dominant mesh submitter at runtime, preserve their generated bodies as super-calls, and retain bounded previous/current instance and mesh arguments for future native producers | `game/render/` (`resident_scene_history.*`), `game/core/toystory2_context.*` | `installResidentSceneObservationOverrides`, `ResidentSceneHistory::beginFrame` | `docs/issues/0030-native-scene-producers-are-not-grounded-at-the-p.md` |
