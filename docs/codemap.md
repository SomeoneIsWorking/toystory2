...
**This repo is scaffolding and the port does not run.** RE-00 is now re-verified: the measured executable
can be placed, freshly imported into Ghidra, cross-checked by two independent methods, and decompiled.
That is RE supply, not a completed game subsystem. There is no recompiled substrate, no
`toystory2_port` binary, no native body, and every `GameConfig` guest address remains zero. **Every ✅
below is TOOLING** with real output recorded in `docs/info/instruments/`; every game subsystem remains
⬜.
...
address this port will ever hold comes out of Ghidra on this executable. RE-00 is complete; RE-01 and
RE-03 are now ready to consume that supply. `docs/references.md` records how the no-decomp negative was
established and that a search negative is weaker than a measurement.
...
| Build | `CMakeLists.txt`, `cmake/toystory2_port.cmake`, framework `tools/check_cpp_style.py` | 🟡 | Verified against recorded psxport `be381503`: `psxport` and `toystory2_seam` (OBJECT lib over all four `game/core` TUs) configure from a bare clone. Normal CTest reuses the framework checker against this repo's tracked Clang policies and real compile DB; there is no per-repo copy or pre-commit hook. `toystory2_port` remains conditional on `generated/rec_sources.cmake`. |
| Play launcher | `run.sh`, `tools/run.py` | 🟡 | `run.sh` is a four-line stable wrapper; Python owns toolchain/framework resolution, provisioning, and the Clang build. The verified default route builds the real seam then exits 3 naming RE-01/RE-02/RE-03; `run_py_selftest` gates its positive and refusal paths. |
...
| crt0 verification tool | — | ⬜ | RE-00 now makes this runnable. The entry decompile shows a clear loop from `0x800A1070` to `0x800D12C0`, but no field is wired from a decompiler guess. RE-01 requires a purpose-built symbolic instrument that prints the instruction behind every field and gates positive, negative and cross-binary cases. |
...
| Everything about the game itself | — | ⬜ | crt0, the overlay loader, the CD path, the frame loop, the pad buffers and the HLE windows remain unimplemented. RE-00 supplies the decompiler; it does not complete any of these. `docs/re-frontier.md` is the ordered authority. |
