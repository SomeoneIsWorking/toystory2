...
eleven ReadN phases, while current pin `8611d756` retains the path under the full compile/static gate.
RE-10 then
...
RE-08, and RE-10 through RE-14/RE-16 are complete; RE-02 and RE-04 are partial. RE-06 pad/input
discovery plus an interactive gameplay witness is the next product frontier.
...
| Process entry | `game/core/main.cpp` | 🟡 | Composition only: installs the process-lifetime `ToyStory2Runtime` and generated registry, initializes framework hardware owners, then delegates override installation and boot through `Core::runtime`. The executed route reaches verified crt0, guest main, FMV, renderer table, stable title, seeded LEVEL01, corrected model-table clear/reload and field 10,303 without a fatal/miss. Interactive input/gameplay remains unverified. |
| Build | `CMakeLists.txt`, `cmake/toystory2_port.cmake`, framework `external/psxport/tools/check_cpp_style.py` | 🟡 | CMake accepts its supported host toolchain; maintainer verification explicitly selects Clang. It builds `toystory2_port`, one shared generated object set, and the independent-oracle boundary runner. CTest owns launcher-contract, RE gates including frame-fence and model-reset controls, boundary compare, format, structure and tidy. Generated sources stay ignored and use their required wrap/aliasing/tail-call flags. A clean Clang build against pin `8611d756` builds both shipping targets and passes 12/12 CTest. |
| Play launcher | `run.sh`, `bootstrap.py`, `tools/run.py`, `tools/test_run.py`, `pyproject.toml`, `uv.lock` | 🟡 | The three-line shell shim enters the frozen uv environment. Python owns resolution, focused 22-module provisioning, hash-checked emission, an isolated `scratch/build/player` product tree with `BUILD_TESTING=OFF`, locked-interpreter propagation, exact native/shader dependency refusals, RmlUi asset path, and the required windowed launch. Ten injected-host tests cover the zero-argument product, prepare-only cold path, absence of CTest, dependency refusals, and compiler pass-through without identity filtering. The direct headless product renders the stable title, enters LEVEL01 and crosses RE-16 through field 10,303; no-argument windowed input/gameplay still needs direct user validation. |
...
| Everything about the game itself | — | 🟡 | The executable substrate, 22-module registry, field delivery/fence, FMV path parsing, resident renderer table, first stable retail title, LEVEL01 entry and RE-16 model-table reset are real. The bounded autonomous route reaches field 10,303 without fatal/miss. Interactive gameplay, OT/packet layout, pad buffers and remaining HLE windows remain incomplete. |
