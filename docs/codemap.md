...
**The real generated port now runs to its first downstream dependency.** RE-00 supplies Ghidra, RE-01
verifies crt0, and RE-03 verifies both resident overlay slots. RE-02 emits the executable and all 21
code modules, links `toystory2_port`, and matches the independent CPU oracle 34/34 at the first call.
...
state was generic CDC following-sector drive pacing, not an unreturned `0x80091DE4` command. Pinned
psxport `3418a79b` removes that race and advances the guest through eleven ReadN phases to the
BITS/MEMORY overlay path at `0x800D9704`/`0x800D95C4` through resident caller `0x8003FA68`.
...
address this port ever holds must come from this executable's binary evidence. RE-00, RE-01 and RE-03
are complete; RE-02 and RE-04 are partial, with the later MEMORY loader boundary next.
...
later (`temisu/ancient`, BSD-2-Clause, the RNC codec) remains deferred until RE-08 needs it;
`docs/references.md` records that decision and its reasons.
...
| Exact overlay map | `tools/overlay_map.py` | ✅ | RE-03 / C010 / I009. Follows FUN_8003D88C through the four-name path builder, fixed level wrapper and synchronous file loader. `--check` derives LEVEL `0x800D12C0`, MEMORY `0x800D5D20`, exact MEMORY end/frontier, and compares GameConfig plus recompiler seeds. Selftest passes 10/10: real positive, stack-relative decoy, refusals, independent jal-target discrimination, shipping comparison, and an in-memory mutation that widens the next slot and forces the opposite `co-resident-possible` verdict. |
...
| Registries | `tools/re_frontier.py` (shim), `tools/info.py`, `tools/catalog.py` | ✅ | The RE-frontier tracker is a SHIM onto the shared engine at `external/psxport/tools/port/re_frontier.py` — this repo grows no fork of it. `check` parses 10 entries: 3 re-verified, RE-02 and RE-04 partial, 0 hacks. C012/I012 and resolved issue #9 record the instruction-derived libcd ABI, the missing CDC timing state, and its generic landed fix. `info.py` and `catalog.py` are COPIES (no hoisted engine exists for them yet) — see the known costs below. |
...
| Static recompilation | `tools/recomp_substrate.py`, `game/recomp_seeds.json` → `generated/` | 🟡 | RE-02/C011/I010. Input-hash-checked shipping emission produces 321 resident roots → 864 functions plus 21 measured overlays / 243 functions / 72 TUs; a separate generated-output digest refuses drift. The first real miss proved IRQ resume `0x80088A2C`, recorded solely as `main_reentry`; pinned psxport `3418a79b` discovers and dispatches that class directly, supplies generic A0:0x15 `strcat`, and accounts guest instructions for deterministic CD pacing. Partial because later computed entries remain hidden behind the current MEMORY-overlay boundary. |
...
| Framework seam — config | `game/core/game_config.cpp` | 🟡 | RE-01's complete boot group, RE-03's two overlay slots, and RE-02's header-derived resident physical range are wired through named constants and shipping/compile-time gates. CD callables stay `0` during the generic raw-controller pacing A/B. RE-04's located `(path,dest)` loader is deliberately not miswired into psxport's incompatible `(dest,lba,size)` field. |
| Framework seam — hooks | `game/core/game_hooks.cpp` | 🟡 | `bootInit` dispatches RE-01's verified gameMain through the generated substrate. Neutral bodies remain only where "nothing is owned" is the correct semantic; fail-fast `abort()` bodies protect framework paths this port has not stood up. |
| Framework seam — recomp registry | `game/core/recomp_register.cpp` | 🟡 | The generated registry now installs resident dispatch/index, all 21 overlay descriptors, and the override setter. No game behavior is native-owned; CD/HLE/pad fields remain separate RE steps. |
| Process entry | `game/core/main.cpp` | 🟡 | Executed. Standard framework bring-up reaches verified crt0, generated gameMain, IRQ registration, disc open and serviced CD DMA. Pinned psxport `3418a79b` advances into the emitted BITS/MEMORY overlay path with no recompilation miss. |
| Build | `CMakeLists.txt`, `cmake/toystory2_port.cmake`, framework `tools/check_cpp_style.py` | 🟡 | Clang-only configure builds `toystory2_port`, one shared generated object set, and the independent-oracle boundary runner. CTest owns launcher, RE-01/RE-03/RE-04, boundary compare, format and tidy gates. Generated sources stay ignored and are compiled with their required wrap/aliasing/tail-call flags. Resolved framework issue #10 records the reproduced shared-source shader race and nested-include regression; `3418a79b` gives each consumer build its own header and permanently gates real nested `add_subdirectory` topology plus the legacy opposite answer. |
| Play launcher | `run.sh`, `tools/run.py` | 🟡 | The stable shell wrapper delegates resolution, focused 21-module provisioning, hash-checked emission, Clang build, RmlUi asset path, and launch to Python. The zero-argument path launches `toystory2_port` against exact pinned psxport `3418a79b`; its bounded CD trace matches the direct route and reaches the emitted BITS/MEMORY overlay. |
...
| Everything about the game itself | — | 🟡 | The executable substrate, overlay registry and first IRQ resume are real. Complete CD, first presentation, frame layout, pad buffers and remaining HLE windows remain unimplemented. `docs/re-frontier.md` is the ordered authority. |
