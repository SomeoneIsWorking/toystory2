...
now verifies and wires the complete crt0 boot group directly from the executable's instructions. There
is still no recompiled substrate, `toystory2_port` binary, or native body. RE-02 is blocked on RE-03's
overlay-loader mechanism, so a verified gameMain address is not a runnable port.
...
address this port ever holds must come from this executable's binary evidence. RE-00 and RE-01 are
complete; RE-03 is the next boot-spine dependency. `docs/references.md` records how the no-decomp
...
| Framework seam — config | `game/core/game_config.cpp` | 🟡 | RE-01's complete boot group is wired through named constants gated by `tools/verify_crt0.py --check`: BSS, stack sources/bias, heap, gp, InitHeap, gameMain and crt0. Optional heap globals are measured absent (`0`). Every other guest address remains honestly `0`; fitted overlay base `0x800D1000` is still not wired because RE-03 has not proved a resident slot. |
...
| Everything about the game itself | — | ⬜ | The crt0 constants are verified, but the overlay loader, substrate, CD path, frame loop, pad buffers and HLE windows remain unimplemented. `docs/re-frontier.md` is the ordered authority. |
