...
**This repo is scaffolding and the port does not run.** RE-00 supplies a verified Ghidra project; RE-01
verifies the complete crt0 boot group, and RE-03 verifies both resident overlay slots directly from the
loader call flow. There is still no recompiled substrate, `toystory2_port` binary, or native body.
RE-02 is now the next ready step; verified addresses alone are not a runnable port.
...
are complete; RE-02 is the next boot-spine dependency. `docs/references.md` records how the no-decomp
...
overlay load bases and the RAM map is NOT static. RE-03/C010 now closes that dependency: the loader
...
| Static recompilation | `game/recomp_seeds.json` → `generated/` | ⬜ | **Not started, now RE-ready.** RE-03 supplies the LEVEL family pattern and BITS__MEMORY exact base. RE-02's executable and re-entry lists remain empty until real substrate dispatch misses justify entries. |
| Framework seam — config | `game/core/game_config.cpp` | 🟡 | RE-01's complete boot group and RE-03's two overlay slots are wired through named constants gated by `tools/verify_crt0.py --check` and `tools/overlay_map.py --check`. Every other guest address remains honestly `0`; RE-04's located `(path,dest)` loader is deliberately not miswired into psxport's incompatible `(dest,lba,size)` field. |
| Framework seam — hooks | `game/core/game_hooks.cpp` | ⬜ | Compiles. Neutral bodies where "nothing is owned" is the correct semantic; fail-fast `abort()` bodies for every framework path this port has not stood up. `bootInit` can dispatch RE-01's verified gameMain only after a substrate exists; its zero guard now detects a config regression. |
| Framework seam — recomp registry | `game/core/recomp_register.cpp` | ⬜ | Deliberately UNWRITTEN. Its explicit no-substrate branch is compiled by `toystory2_seam`; it `#error`s under `TS2_HAVE_SUBSTRATE` so a substrate cannot appear without someone writing the real generated-symbol registry. |
...
| Build | `CMakeLists.txt`, `cmake/toystory2_port.cmake`, framework `tools/check_cpp_style.py` | 🟡 | Verified against recorded psxport `2b5ef7b5`: `psxport` and `toystory2_seam` (OBJECT lib over all four `game/core` TUs) configure from a bare clone. Normal CTest runs the launcher test, RE-01 verifier, RE-03 overlay-map verifier, and shared framework checker against this repo's tracked Clang policies and real compile DB; there is no per-repo C++ checker copy or pre-commit hook. `toystory2_port` remains conditional on `generated/rec_sources.cmake`. |
| Play launcher | `run.sh`, `tools/run.py` | 🟡 | `run.sh` is a four-line stable wrapper; Python owns toolchain/framework resolution, provisioning, and the Clang build. The default route builds the real seam then exits 3 naming only RE-02; `run_py_selftest` gates its positive and refusal paths. |
...
| Everything about the game itself | — | ⬜ | The crt0 constants and overlay loader map are verified, but the substrate, complete CD path, frame loop, pad buffers and HLE windows remain unimplemented. `docs/re-frontier.md` is the ordered authority. |
