...
The first empirical resume seed (`0x80088A2C`) is classified and wired; live boot now opens the disc and
waits in stock-libcd command code. RE-04's ABI-checked CD seam is the earliest missing boot stage.
...
are complete; RE-02 is partial and RE-04 is the next boot-spine dependency. `docs/references.md` records how the no-decomp
...
| Registries | `tools/re_frontier.py` (shim), `tools/info.py`, `tools/catalog.py` | ✅ | The RE-frontier tracker is a SHIM onto the shared engine at `external/psxport/tools/port/re_frontier.py` — this repo grows no fork of it. After RE-02, `check` parses 10 entries: 3 re-verified, RE-02 and RE-04 partial, 0 hacks. C011 plus I010/I011 record the substrate evidence and falsifiers. `info.py` and `catalog.py` are COPIES (no hoisted engine exists for them yet) — see the known costs below. |
...
| Static recompilation | `tools/recomp_substrate.py`, `game/recomp_seeds.json` → `generated/` | 🟡 | RE-02/C011/I010. Input-hash-checked shipping emission produces 321 resident roots → 864 functions plus 21 measured overlays / 243 functions / 72 TUs; a separate generated-output digest refuses drift. The first real miss proved IRQ resume `0x80088A2C`, recorded solely as `main_reentry`; pinned psxport 692b9b20 discovers and dispatches that class directly and supplies the measured generic A0:0x15 `strcat` leaf. Partial because later computed entries remain hidden behind the current stock-libcd completion boundary. |
| Framework seam — config | `game/core/game_config.cpp` | 🟡 | RE-01's complete boot group, RE-03's two overlay slots, and RE-02's header-derived resident physical range are wired through named constants and shipping/compile-time gates. Every other guest address remains honestly `0`; RE-04's located `(path,dest)` loader is deliberately not miswired into psxport's incompatible `(dest,lba,size)` field. |
...
| Process entry | `game/core/main.cpp` | 🟡 | Executed. Standard framework bring-up reaches verified crt0, generated gameMain, IRQ registration and disc open. It then honestly waits for RE-04's missing CD completion seam. |
| Build | `CMakeLists.txt`, `cmake/toystory2_port.cmake`, framework `tools/check_cpp_style.py` | 🟡 | Clang-only configure builds `toystory2_port`, one shared generated object set, and the independent-oracle boundary runner. CTest owns launcher, RE-01/RE-03, boundary compare, format and tidy gates. Generated sources stay ignored and are compiled with their required wrap/aliasing/tail-call flags. |
| Play launcher | `run.sh`, `tools/run.py` | 🟡 | The stable shell wrapper delegates resolution, focused 21-module provisioning, hash-checked emission, Clang build, RmlUi asset path, and launch to Python. The zero-argument path now launches `toystory2_port`; its current honest runtime boundary is RE-04's CD wait. |
| Generated boundary gate | `tools/compare_recomp_boundary.py`, `tests/toystory2_recomp_boundary.cpp` | ✅ | Executes the identity-checked entry through shipping generated C and the independent Mednafen CPU oracle to first jal `0x80089344`: 34/34 state fields agree. Forced `a0` mutation gives one named mismatch, proving the comparator can report the opposite answer. |
