...
| Guest execution adapter | Centralize typed guest calls, bounded finite initialization continuation, scoped original calls and resident override registration | `game/core/guest_execution.*` | `callGuestToReturn`, `callFiniteGuestToReturn`, `executeFiniteGuestCall`, `installResidentOverride` | `CLAUDE.md` |
| Shared MEMORY/FMV code slot | Observe the measured file loader, authenticate each retail source before transfer and its loaded bytes afterward, retire replaced identity, invalidate translations, and publish one image generation | `game/overlay/shared_slot_image.*` | `installSharedSlotImageObserver`, `SharedSlotImage::publish` | `docs/re-frontier.md` |
