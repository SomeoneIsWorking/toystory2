...
**The real generated port now renders the legal/ESRB boot cards headlessly and enters FMV code.** RE-00
...
psxport `3418a79b` removes that race and advances the guest through eleven ReadN phases. RE-10 then
...
emitted FMV at `0x800D6628` and stops honestly at missing shared BIOS `A0:0x25`.
...
address this port ever holds must come from this executable's binary evidence. RE-00, RE-01, RE-03 and
RE-10 are complete; RE-02 and RE-04 are partial, with shared BIOS `A0:0x25` the live boundary.
...
| Registries | `tools/re_frontier.py` (shim), `tools/info.py`, `tools/catalog.py` | ✅ | The RE-frontier tracker is a SHIM onto the shared engine at `external/psxport/tools/port/re_frontier.py` — this repo grows no fork of it. `check` parses 13 entries: 5 re-verified, RE-02 and RE-04 partial, 0 hacks. C016/resolved issue #13 record the landed FMV BIOS fix; issue #14 records the next generic computed-jump-table defect. `info.py` and `catalog.py` are COPIES (no hoisted engine exists for them yet) — see the known costs below. |
...
| Static recompilation | `tools/recomp_substrate.py`, `game/recomp_seeds.json` → `generated/` | 🟡 | RE-02/C014/I010. Input-hash-checked shipping emission produces 366 resident roots → 890 functions plus 22 modules / 308 functions / 75 TUs; a separate digest refuses drift. FMV entry `0x800D6628` is binary/live proven. Renderer re-entry `0x8001040C` is an exact reproducer, not completion: the verifier proves it is one of 32 internal slots and live execution reaches sibling `0x800104E4` because generic lowering is absent. |
...
| Framework seam — runtime | `game/core/toystory2_runtime.*`, `game/core/legacy_game_interface.h` | 🟡 | `ToyStory2Runtime` is the process-lifetime derived behavior owner. It directly owns RE-01 boot dispatch and RE-10 override installation. The `LegacyGameRuntimeAdapter` base is explicit debt until generic framework readers replace the remaining config facts and compatibility callbacks with narrow typed interfaces. |
| Framework seam — legacy facts | `game/core/game_config.cpp`, `game/core/game_hooks.cpp` | 🟡 | RE-01's complete boot group, RE-03's two overlay slots, and RE-02's header-derived resident physical range remain in the bounded compatibility table because generic framework algorithms still read `Core::cfg`. CD callables stay `0` during the raw-controller pacing A/B; the measured `(path,dest)` loader is not miswired into psxport's incompatible `(dest,lba,size)` field. Hooks now contain only neutral or fail-fast compatibility behavior. |
| Framework seam — field clock | `game/sync/` (`field_clock.*`), `game/core/toystory2_runtime.cpp` | ✅ | RE-10/C013/I013. The runtime installs the exact graphics-init override; after guest graphics init it arms host turns at the GPU field rate. Each turn samples pad, invokes registered guest VBlank `0x80039D60`, advances SPU and presents. Live A/B changes a no-present watchdog into visible retail boot cards. |
...
| Process entry | `game/core/main.cpp` | 🟡 | Composition only: installs the process-lifetime `ToyStory2Runtime` and generated registry, initializes framework hardware owners, then delegates override installation and boot through `Core::runtime`. The executed route reaches verified crt0, guest main, visible boot cards and emitted FMV before shared BIOS `A0:0x25`. |
| Build | `CMakeLists.txt`, `cmake/toystory2_port.cmake`, framework `external/psxport/tools/check_cpp_style.py` | 🟡 | Clang-only configure builds `toystory2_port`, one shared generated object set, and the independent-oracle boundary runner. CTest owns launcher, RE-01/RE-03/RE-04, boundary compare, format and tidy gates. Generated sources stay ignored and are compiled with their required wrap/aliasing/tail-call flags. The recorded framework pin is `7f5d3f13`, whose derived-runtime seam and per-consumer generated shader header are exercised by this build. |
| Play launcher | `run.sh`, `tools/run.py` | 🟡 | The stable shell wrapper delegates resolution, focused 22-module provisioning, hash-checked emission, Clang build, RmlUi asset path, and launch to Python. The executable's direct headless route renders boot cards and enters FMV; the launcher remains to be revalidated after the current shared-runtime work lands. |
...
| Everything about the game itself | — | 🟡 | The executable substrate, 22-module registry, field delivery and first visible boot presentations are real. The next boundary is shared BIOS `A0:0x25`; gameplay, OT/packet layout, pad buffers and remaining HLE windows remain incomplete. |
