...
`3418a79b` first removed that race; current pin `57a17a14` retains it and advances the guest through
eleven ReadN phases. RE-10 then
...
emitted FMV at `0x800D6628`, completes the shared BIOS `A0:0x25` path parser, traverses the exact
32-way resident renderer table and reaches RenderQueue capture capacity.
...
RE-10, RE-11 and RE-12 are complete; RE-02 and RE-04 are partial, with RenderQueue capture capacity
the live boundary.
...
| Registries | `tools/re_frontier.py` (shim), `tools/info.py`, `tools/catalog.py` | ✅ | The RE-frontier tracker is a SHIM onto the shared engine at `external/psxport/tools/port/re_frontier.py` — this repo grows no fork of it. `check` parses 13 entries: 6 re-verified, RE-02 and RE-04 partial, 0 hacks. C016/resolved issue #13 record the landed FMV BIOS fix; C017/resolved issue #14 record the landed exact jump-table lowering. `info.py` and `catalog.py` are COPIES (no hoisted engine exists for them yet) — see the known costs below. |
...
| Static recompilation | `tools/recomp_substrate.py`, `game/recomp_seeds.json` → `generated/` | 🟡 | RE-02/C014/C017/I010. Input-hash-checked shipping emission produces 365 resident roots → 889 functions plus 22 modules / 308 functions / 75 TUs; a separate digest refuses drift. FMV entry `0x800D6628` is binary/live proven. No renderer table slot is seeded. Pinned psxport `57a17a14` generically derives exactly 32 local cases from `lui+ori`, the scale and mask; the real route passes both former 0x8001040C/0x800104E4 misses and reaches RenderQueue capacity. |
...
| Process entry | `game/core/main.cpp` | 🟡 | Composition only: installs the process-lifetime `ToyStory2Runtime` and generated registry, initializes framework hardware owners, then delegates override installation and boot through `Core::runtime`. The executed route reaches verified crt0, guest main, visible boot cards, emitted FMV, the shared BIOS `A0:0x25` leaf and the exact resident renderer table before RenderQueue capture capacity. |
| Build | `CMakeLists.txt`, `cmake/toystory2_port.cmake`, framework `external/psxport/tools/check_cpp_style.py` | 🟡 | Clang-only configure builds `toystory2_port`, one shared generated object set, and the independent-oracle boundary runner. CTest owns launcher, RE-01/RE-03/RE-04, boundary compare, format and tidy gates. Generated sources stay ignored and are compiled with their required wrap/aliasing/tail-call flags. The recorded and built framework commit is `57a17a14`, whose shared BIOS and exact computed-jump-table lowering are exercised by the real-disc gates. |
| Play launcher | `run.sh`, `tools/run.py` | 🟡 | The stable shell wrapper delegates resolution, focused 22-module provisioning, hash-checked emission, Clang build, RmlUi asset path, and launch to Python. The executable's direct headless route renders boot cards and reaches RenderQueue capture capacity; the no-argument launcher still needs a bounded visual revalidation at the current pin. |
...
| Everything about the game itself | — | 🟡 | The executable substrate, 22-module registry, field delivery, first visible boot presentations, FMV path parsing and resident renderer jump table are real. The next measured boundary is RenderQueue capture capacity; gameplay, OT/packet layout, pad buffers and remaining HLE windows remain incomplete. |
