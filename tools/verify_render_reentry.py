...
This is a consumer-side guard for a ``main_reentry`` seed.  It proves from the
shipping executable that the address is an in-function jump-table trampoline,
not a callable function root or an arbitrary function-pointer destination.
"""
...
SIBLING_CALLER_RA = 0x80014C8C
SIBLING_CALLER_PC = 0x80014D3C
...
def check(exe_path: Path, seeds_path: Path) -> None:
    analyze(exe_path.read_bytes())
    seeds = load_seeds(seeds_path)
    address = f"0x{REENTRY:08X}"
    if address not in seeds.get("main_reentry", []):
        raise Refused(f"{address} is absent from main_reentry")
    if address in seeds.get("main", []):
        raise Refused(f"{address} is incorrectly duplicated as a function root")
    print(
        "[render-reentry] retail jr table selects internal trampoline "
        f"0x{REENTRY:08X} -> 0x{DESTINATION:08X}; main_reentry is the sole seed class"
    )


def check_reproducer_log(log: str) -> None:
    expected = re.compile(
        rf"\[recomp-MISS 0\] no recompiled fn for 0x{SIBLING_REENTRY:08X}\s+"
        rf"\(caller ra=0x{SIBLING_CALLER_RA:08X}, .*?c->pc=0x{SIBLING_CALLER_PC:08X}\)"
    )
    if not expected.search(log):
        raise Refused(
            "trace lacks the exact post-0x8001040C sibling-table miss at "
            f"0x{SIBLING_REENTRY:08X}"
        )
    if "unimplemented BIOS A0:0x25" in log:
        raise Refused("trace still contains the retired shared BIOS A0:0x25 boundary")
    print(
        "[render-reentry] live single-slot reproducer reached sibling "
        f"0x{SIBLING_REENTRY:08X} from pc=0x{SIBLING_CALLER_PC:08X}"
    )
...
def selftest(exe_path: Path) -> int:
    ...
    try:
        check(exe_path, DEFAULT_SEEDS)
        tests.append(("positive retail internal trampoline and sole seed class", True))
    except (OSError, Refused, json.JSONDecodeError):
        tests.append(("positive retail internal trampoline and sole seed class", False))
    ...
    exact_log = (
        "[hle:warn] [recomp-MISS 0] no recompiled fn for 0x800104E4  "
        "(caller ra=0x80014C8C, a0=0x8015A4E4, c->pc=0x80014D3C)\n"
    )
    try:
        check_reproducer_log(exact_log)
        tests.append(("positive exact sibling live boundary", True))
    except Refused:
        tests.append(("positive exact sibling live boundary", False))
    try:
        check_reproducer_log(exact_log.replace("0x800104E4", "0x8001040C"))
        tests.append(("negative old first-slot boundary", False))
    except Refused:
        tests.append(("negative old first-slot boundary", True))
...
def main() -> int:
    ...
    try:
        ...
        check(args.exe, args.seeds)
        if args.check_log:
            check_reproducer_log(
