...
COPYRIGHT_PATTERNS = [
    ...
    # THIS repo: the disc is Activision's/Traveller's Tales', and so is SLUS_008.93. They are PATH
    # patterns (check A), which is why naming the executable here is safe while naming it in
    # .gitignore is not — check C turns every gitignore pattern into a TEXT search token, and every
    # doc in this repo names SLUS_008.93 in prose.
    r"\.chd$", r"\.cue$", r"\.ecm$", r"\.pbp$", r"\.m3u$",
    r"(^|/)SLUS_008\.93$", r"(^|/)SLUS_00893$",
    # This game's asset extensions are deliberately ABSENT. TS2's disc is .BIN/.RAW/.DAT/.ALL/.ANM —
    # extensions so generic that a path pattern for them would flag ordinary build output and any
    # future test fixture. What must never be committed here is anything EXTRACTED from the disc, and
    # everything extracted lands under the gitignored scratch/ (check A still sees a path that ever
    # existed in history, which is the case this list exists for).
]
